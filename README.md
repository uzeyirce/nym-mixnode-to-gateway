...
# <h1 align="center">MIXNODE TO GW</h1>
...
In this guide, we will demonstrate how to transition your node from Nym node mixnode mode to Nym node gateway exit and entry modes.


Additionally, this guide https://github.com/uzeyirce/nym-node-gateway-mode/blob/main/README.md will provide detailed instructions on setting up the Nym node gateway (exit-entry) mode from scratch.

First, use the following commands to check your Nym node's node ID and bonding information while it's running in mixnode mode:

Node ID:
```bash
ls -la $HOME/.nym/nym-nodes
```

Bonding Information:
```bash
./nym-node bonding-information --id YOURID
```

Let's stop the running node.
```bash
service nym-node stop
```

Allow the following port for WireGuard.
```bash
...
Enter the Node ID of your existing mixnode below. Then, enter the hostname in the hostname field.
Instructions on how to create a hostname are provided in this guide. (https://github.com/uzeyirce/nym-node-gateway-mode/blob/main/README.md) 
```bash
./nym-node run --id NODEID --init-only --mode entry-gateway --public-ips "$(curl -4 https://ifconfig.me)" --hostname "HOSTNAME" --http-bind-address 0.0.0.0:8080 --mixnet-bind-address 0.0.0.0:1789 --accept-operator-terms-and-conditions --wireguard-enabled true
```

```bash
./nym-node run --id NODEID --init-only --mode exit-gateway --public-ips "$(curl -4 https://ifconfig.me)" --hostname "HOSTNAME" --http-bind-address 0.0.0.0:8080 --mixnet-bind-address 0.0.0.0:1789 --accept-operator-terms-and-conditions --wireguard-enabled true
```

Next, go to the wallet where your node is running, add the hostname in the node settings, and save it.
```bash
...
In the config file, find the following sections and update them  as shown below.
```bash
...
enabled = true
landing_page_assets_path = '/var/www/HOSTNAME'
...
```bash
nano /var/www/HOSTNAME/index.html
```
...
```bash
certbot --nginx --non-interactive --agree-tos --redirect -m EMAILADDRESS -d HOSTNAME
```
